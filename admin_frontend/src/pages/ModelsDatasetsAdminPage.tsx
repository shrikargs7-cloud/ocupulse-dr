...
export const ModelsPage: React.FC = () => {
  const [activeDatasetTab, setActiveDatasetTab] = useState<'aptos' | 'kaggle_dr' | 'idrid' | 'drive' | 'messidor'>('aptos');
  ...
  const datasets = {
    ...
    kaggle_dr: {
      name: 'Diabetic Retinopathy Balanced 5-Class Dataset',
      host: 'Kaggle (by Sachin Kumar)',
      size: '2,750 Clinically Categorized Fundus Images',
      role: '5-Class ICDR Multi-Grade Classification & Class Imbalance Evaluation',
      link: 'https://www.kaggle.com/datasets/sachinkumar413/diabetic-retinopathy-dataset?resource',
      details: 'Curated 5-class diabetic retinopathy dataset comprising 2,750 fundus images stratified across all severity stages. Provides distinct distributions for evaluating class imbalance robustness and ordinal classification in clinical AIDRSS workflows.',
      classDistribution: [
        { label: 'Healthy (Not DR)', count: 1000, percentage: '36.4%', color: 'bg-emerald-500 text-emerald-400 border-emerald-500/30' },
        { label: 'Mild DR', count: 370, percentage: '13.5%', color: 'bg-blue-500 text-blue-400 border-blue-500/30' },
        { label: 'Moderate DR', count: 900, percentage: '32.7%', color: 'bg-amber-500 text-amber-400 border-amber-500/30' },
        { label: 'Severe DR', count: 190, percentage: '6.9%', color: 'bg-orange-500 text-orange-400 border-orange-500/30' },
        { label: 'Proliferative DR', count: 290, percentage: '10.5%', color: 'bg-rose-500 text-rose-400 border-rose-500/30' }
      ]
    },
    idrid: {
      ...
      >
        <div className="flex flex-wrap items-center justify-between gap-4 border-b border-slate-800 pb-4">
          ...
          <div className="flex flex-wrap bg-slate-900 border border-slate-800 p-1 rounded-xl gap-1">
            {[
              ...
              { key: 'kaggle_dr', label: 'Kaggle DR (2,750)' },
        ...
        {datasets[activeDatasetTab] && (
          <div className="p-5 rounded-2xl bg-slate-900/60 border border-slate-800 space-y-4">
            ...
            {/* Class distribution breakdown if available */}
            {(datasets[activeDatasetTab] as any).classDistribution && (
              <div className="pt-4 border-t border-slate-800/80 space-y-3">
                <div className="flex items-center justify-between">
                  <span className="text-[11px] font-mono uppercase text-slate-400 tracking-wider flex items-center gap-1.5">
                    ...
                    Dataset Stratification (5 Class Labels • 2,750 Total Images):
                  </span>
                  <span className="text-[11px] font-mono text-emerald-400 font-semibold">100% Verified</span>
                </div>

                <div className="grid grid-cols-1 sm:grid-cols-5 gap-3">
                  {(datasets[activeDatasetTab] as any).classDistribution.map((item: any, iIdx: number) => (
                    <div key={item.label} className="p-3 rounded-xl bg-slate-950/80 border border-slate-800 space-y-1.5">
                      <div className="flex items-center justify-between text-[11px] font-mono">
                        <span className="text-slate-400 font-semibold">{iIdx + 1}. {item.label}</span>
                      </div>
                      <div className="flex items-baseline justify-between">
                        <span className="text-lg font-bold font-mono text-white">{item.count.toLocaleString()}</span>
